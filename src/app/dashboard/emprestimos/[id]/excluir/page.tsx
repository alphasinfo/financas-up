...
export default function ExcluirEmprestimoPage({ params }: { params: { id: string } }) {
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      const res = await fetch(`/api/emprestimos/${params.id}`, { method: "DELETE" });
      if (!res.ok) throw new Error();
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-3xl font-bold">Excluir Empréstimo</h1>
      </div>
      <Card className="border-red-200 bg-red-50">
        ...
        <CardContent className="space-y-4">
          <div className="bg-white rounded-lg p-4">
            <h3 className="font-bold text-lg">{emprestimo.instituicao}</h3>
            {emprestimo.descricao && <p className="text-gray-600">{emprestimo.descricao}</p>}
            <div className="grid grid-cols-3 gap-4 pt-3 border-t mt-3">
              <div>
                <p className="text-sm text-gray-500">Valor Total</p>
                <p className="font-bold">R$ {emprestimo.valorTotal.toFixed(2)}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Pago</p>
                <p className="font-bold text-green-600">R$ {totalPago.toFixed(2)}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Restante</p>
                <p className="font-bold text-red-600">R$ {totalRestante.toFixed(2)}</p>
              </div>
            </div>
          </div>
          <div className="bg-red-100 border border-red-300 rounded-lg p-4">
            <p className="text-red-900 text-sm">
              <strong>Aviso:</strong> Todos os dados e {emprestimo.parcelas?.length || 0} parcela(s) serão permanentemente removidos.
            </p>
          </div>
